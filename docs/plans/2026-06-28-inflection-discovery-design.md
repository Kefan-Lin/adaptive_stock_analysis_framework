...
- **Forward return** — 6- and 12-month forward return of the top-N basket at each as-of date vs the universe average. **Dead-name handling:** a name that delists/goes to zero in the window contributes its realized loss (down to −100%), never silently drops; a **30+ day trading gap inside the window** is treated as death of the original line and the return is realized at the **last pre-gap close**, so a recycled ticker's successor prices can never mask the collapse (audit I2). **Liquidity haircut (concrete rule):** a name whose **trailing-60-session median dollar volume at the as-of date is below $1M** (`MIN_ADV_USD`) is not costlessly investable — it is **excluded from top-N eligibility and counted in `excluded_illiquid`** (reported beside `excluded_no_data`); a `None` ADV (unverifiable) is treated as excluded, never guessed. This removes microcap "paper" returns (AXTI, FFAI, BLNK, FOSL) from the tradable cut (audit I1).
